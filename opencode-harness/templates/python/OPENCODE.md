...
- **Missing Binaries:** If `./run-checks.sh` fails because a binary (like `ruff` or `pytest`) is missing, you MUST install it (e.g., `pip install -r requirements.txt` or create a virtual environment) and rerun the checks. Do not skip checks.
